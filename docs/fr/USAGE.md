...
![alt text](<../../media/barre kablix.webp>)
  ...
  - le **mode texte** (l'icône « T ») : pose des **étiquettes** libres sur la feuille — un titre de montage, une remarque, le nom d'une zone. Un clic sur l'icône l'enfonce ; un clic n'importe où sur la feuille pose une étiquette et ouvre sa saisie (plusieurs lignes autorisées, la zone s'étend avec le texte). Le mode se quitte en recliquant l'icône, avec Échap, ou dès qu'on clique autre chose (un composant, un fil, un bouton, la bibliothèque) — et si Kablix perd le focus. Le curseur porte alors un **T**, pour rappeler que le prochain clic pose une étiquette. Hors mode texte, une étiquette se **déplace** comme un composant et se supprime avec **Suppr** ; en mode texte, un clic dessus rouvre sa saisie (copier / coller acceptés — le texte collé arrive toujours **en clair**, sans la mise en forme de sa provenance). Une étiquette **sélectionnée** se règle dans le panneau de droite : **couleur du texte**, **couleur du fond**, **transparence du fond** (jusqu'à zéro : le texte reste seul, sans pavé de couleur), **taille du texte** et **police**. Les étiquettes ne sont que du décor : la simulation les ignore, mais l'export SVG les emporte, réglages compris.
